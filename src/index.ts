...
export const ImprovedTodowritePlugin: Plugin = ({ client }) => {
	...
	function runAndPublish(
		...
		return runTodowrite(sessionID, toolName, args).then((raw) => {
			...
			return publishTodoTree(sessionID, result).then(() =>
				setDisplay(setMeta, result.display),
			);
		});
	}
			...
			todo_plan: tool({
				...
				execute(args: { todos: unknown[] }, context) {
					return context
						...
						.then(() =>
							runAndPublish(context.sessionID, "todo-plan", args, (opts) =>
			...
			todo_read: tool({
				...
				execute(_args: Record<string, never>, context) {
					return context
						...
						.then(() =>
							runAndPublish(context.sessionID, "todo-read", {}, (opts) =>
								context.metadata(opts),
							),
						);
				},
			...
			todo_edit: tool({
				...
				execute(args: { ops: unknown[] }, context) {
					return context
						...
						.then(() =>
							runAndPublish(context.sessionID, "todo-edit", args, (opts) =>
								context.metadata(opts),
							),
						);
				},
